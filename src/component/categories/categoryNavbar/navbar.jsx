    ...
    return (
        <>
            <div className={st.category_navbar} style={{background: dark ? '#0C0C0D' : '#F8F9FC'}}>
                ...
                <div className={st.items} style={{borderTop: type === 'genres' && dark  ? '1px solid #777777' : '1px solid #1111113f'}}>
                    {
                        type === 'genres' ? data && data.map((val, key) => <Link
                        to={`/${language.lang || 'ru'}/genres/${val.genre_name.toLowerCase()}`}
                        id={val.genre_id}
                        ...
                        key={key}
                        to={`/${language.lang || 'ru'}/categories/${val.category_name.toLowerCase()}`}
                        id={val.category_id}
                        onClick={e => setActive(e.target.id)}
                        style={{
                            background: dark ? 'rgba(17, 17, 18, 1)' : '#fff',
